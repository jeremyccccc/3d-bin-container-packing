...
class WholeOrderPackingTest {
	...
	@Test
	void rejectsSplitResultWhenNoWholeOrderAssignmentExists() throws Exception {
		PackingResponse response = service.pack(read("""
				{"masterBsId":"M3",
				 "containerLists":[{"id":"C1","size":20,"type":"GP"},{"id":"C2","size":20,"type":"GP"}],
				 "houseBillList":[
		...
		assertThat(response.containerLists()).allSatisfy(container -> assertThat(container.allocatedHouseBillList()).isEmpty());
	}
